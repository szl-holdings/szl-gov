...
def main() -> int:
    ...
    claims = [
        ...
        },
        {
            ...
            "truth_state": "VERIFIED",
            "evidence": ["outreach/targets.md", "outreach/README.md"],
            "notes": "4 personalized emails SENT 2026-08-30: AICamp (DP), VerifyWise + Modulos (standards co-emission), Alpha Governance (founding cohort). Replies tracked by daily inbox watch.",
        },
    ]
    ...
    contradictions = [
        ...
        {"id": "HW-003", "severity": "BLOCKER",
         "statement": "a-11-oy.com and www.a-11-oy.com now fail TLS: DNS was repointed off Cloudflare directly at Hugging Face IPs, but the custom domain is not registered on the Space, so HF serves its default CN=hf.space certificate. Browsers show a certificate warning on the product domain. Underlying Space szlholdings-a11oy.hf.space is healthy (200).",
         "resolution": "Register a-11-oy.com AND www.a-11-oy.com as custom domains in SZLHOLDINGS/a11oy Space settings so HF provisions a matching certificate, or revert DNS to Cloudflare. Verified 2026-09-01: cert subject CN=hf.space, issuer Amazon RSA 2048 M01."},
        {"id": "HW-002", "severity": "MEDIUM",
         "statement": "Hardware acceptance criteria are not yet bound to the software verifier",
         "resolution": "Binding spec written: docs/hardware/RC1_ACCEPTANCE_BINDING.md (RC1-ACC-1..3, 10-row field map, 8 negative cases incl. N8 bypass test). Clears when the clause is merged into RFQ Rev 1.0 sections 7/17/21."},
    ]
